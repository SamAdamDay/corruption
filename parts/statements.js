...
0;for ( i = 0 ; i < ( z && v . UP [ 1 ] / 100 ) ; i ++ ) { b = c ( s ) ; p = b . indexOf ( P ) ; x = p % W ; y = ( p - x ) / W ; y = Math . max ( 0 , y - 1 ) ; m = b [ y * W + x - 1 ] ; if ( m != B ) { if ( /[ÄËÏÖÜ]/ . test( m ) ) { f += J } if ( m == C ) { f += I } t = b . replace ( P , S ) ; s = d ( t . slice ( 0 , y * W + x - 1 ) + P + t . slice ( y * W + x ) ); } } z = z && ! i ; _c = 1;0
// Move down if the down key is pressed [has defensive armour]
0;for ( i = 0 ; i < ( z && v . DOWN [ 1 ] / 100 ) ; i ++ ) { b = c ( s ) ; p = b . indexOf ( P ) ; x = p % W ; y = ( p - x ) / W ; y = Math . min ( Math . ceil ( b . length / W ) - 1 , y + 1 ) ; m = b [ y * W + x + 1 ] ; if ( m != B ) { if ( /[ÄËÏÖÜ]/ . test( m ) ) { f += J } if ( m == C ) { f += I } t = b . replace ( P , S ) ; s = d ( t . slice ( 0 , y * W + x + 1 ) + P + t . slice ( y * W + x + 2 ) ); } } z = z && ! i ; _c = 1;0
// Move left if the left key is pressed [has defensive armour]
0;for ( i = 0 ; i < ( z && v . LEFT [ 1 ] / 100 ) ; i ++ ) { b = c ( s ) . replace ( /\n/g , '' ) ; p = b . indexOf ( P ) ; x = p % W ; y = ( p - x ) / W ; x = Math . max ( 0 , x - 1 ) ; m = b [ y * W + x ] ; if ( m != B ) { if ( /[ÄËÏÖÜ]/ . test( m ) ) { f += J }  if ( m == C ) { f += I } t = b . replace ( P , S ) ; s = d ( t . slice ( 0 , y * W + x ) + P + t . slice ( y * W + x + 1 ) ); } } z = z && ! i ; _c = 1;0
// Move right if the right key is pressed [has defensive armour]
0;for ( i = 0 ; i < ( z && v . RIGHT [ 1 ] / 100 ) ; i ++ ) { b = c ( s ) . replace ( /\n/g , '' ) ; p = b . indexOf ( P ) ; x = p % W ; y = ( p - x ) / W ; x = Math . min ( W - 1 , x + 1 ) ; m = b [ y * W + x ] ; if ( m != B ) { if ( /[ÄËÏÖÜ]/ . test( m ) ) { f += J }  if ( m == C ) { f += I } t = b . replace ( P , S ) ; s = d ( t . slice ( 0 , y * W + x ) + P + t . slice ( y * W + x + 1 ) ); } } z = z && ! i ; _c = 1;0
