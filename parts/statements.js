...
// Randomly add a road-block; LENGTH HARDCODED AS `S + S + S + S + S`
 if ( r ( ) < 0.1 * Math . pow ( Math . max ( v . UP [ 1 ] , v . DOWN [ 1 ] , v . LEFT [ 1 ] , v . RIGHT [ 1 ] ) / Q , 2 ) ) { b = c ( s ) . split ( t = S + S + S + S + S ); k = u ( r ( ) * ( b . length - 2 ) ) ; s = d ( b . slice ( 0 , k ) . concat ( b [ k ] + R  + b [ k + 1 ], b . slice ( k + 2 ) ) . join ( t ) ) ; if ( r ( ) < 0.1 ) b = c ( s ) . split ( C ) , k = u ( r ( ) * ( b . length - 2 ) ) , b . splice ( k , 2 , b [ k ] + B + b [ k + 1 ] ) , s = d ( b . join ( C ) ) } _z = 13 
...
// Randomly swap two adjacent statements
 if ( r ( ) < 0.001 ) { k = u ( r ( ) * ( s . length - 2 ) ) ; t = s [ k ] ; s [ k ] = s [ k + 1 ] ; s [ k + 1 ] = t } _z = 2 
...
// Randomly add a gold piece; LENGTH HARDCODED AS `S + S + S + S + S + S`
 if ( r ( ) < 0.06 ) { b = c ( s ) . split ( t = S + S + S + S + S + S ); k = u ( r ( ) * ( b . length - 2 ) ) ; s = d ( b . slice ( 0 , k ) . concat ( b [ k ] + G  + b [ k + 1 ], b . slice ( k + 2 ) ) . join ( t ) ) ; } _z = 6 
// Randomly reduce the score
 if ( r ( ) < 0.1 ) { f-- } _z = 7 
// The virus; it self replicates in available space, and eats gold (length 147)
s=r()<.5?d(c(s).replace(/\s{155}/m,D+s[_i]+D)):s;if(r()<.07)b=c(s).split(C),k=u(r()*(b.length-2)),b.splice(k,2,b[k]+'#'+b[k+1]),s=d(b.join(C));_z=8
...
// The monster-controller; it moves umlaut-monsters left and right
 a1 = '\\/' ; a2 = '\\*' ; t = C + a1 + a2 + S + B + 'ÄËÏÖÜ#><' ; s = d ( c ( s ) . replace ( new RegExp ( '([^' + t + '])' + a1 + a2 + '([ÄËÏÖÜ])<[ÄËÏÖÜ]' + a2 + a1 , 'g' ) , '$1/*$2>$2*/' ) . replace ( new RegExp ( '([' + t + '])' + a1 + a2 + '([ÄËÏÖÜ])<[ÄËÏÖÜ]' + a2 + a1 , 'g' ) , '/*$2<$2*/' + S ) . replace ( new RegExp ( a1 + a2 + '([ÄËÏÖÜ])>[ÄËÏÖÜ]' + a2 + a1 + '([^' + t + '])' , 'g' ) , '/*$1<$1*/$2' ) . replace ( new RegExp ( a1 + a2 + '([ÄËÏÖÜ])>[ÄËÏÖÜ]' + a2 + a1 + '([' + t + '])' , 'g' ) , S+'/*$1>$1*/' ) ) ; _z = 10 
